...
import { Terminal, Wallet, ArrowRight, Database, Coins, HardDrive, Share2, Key, ShieldCheck, ArrowDown, Search, FileText, Lock, UserPlus, Snowflake, Anchor, Cpu, Play, Pause, ChevronLeft, ChevronRight, RotateCcw, CheckCircle } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-950 text-white font-sans overflow-hidden">
      <div className="h-14 bg-gray-900 border-b border-gray-800 flex items-center px-4 justify-between shrink-0">
        ...
        <div className="flex items-center gap-2">
            <button onClick={handlePrev} disabled={currentStep === 0 || isTyping} className="p-2 text-gray-400 hover:text-white hover:bg-gray-800 rounded-lg disabled:opacity-30 disabled:cursor-not-allowed transition-colors" title="Previous Step">
                <ChevronLeft className="w-5 h-5" />
            </button>
            <button onClick={togglePlay} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg font-bold text-xs transition-all ${isPlaying ? 'bg-yellow-600/20 text-yellow-400 border border-yellow-600/50' : 'bg-green-600 hover:bg-green-500 text-white'}`} title={isPlaying ? "Pause" : "Auto Play"}>
                {isPlaying ? <Pause className="w-4 h-4 fill-current" /> : <Play className="w-4 h-4 fill-current" />}
                <span className="hidden sm:inline">{isPlaying ? 'Pause' : 'Auto Play'}</span>
            </button>
            <button onClick={handleNext} disabled={isTyping || isFinished} className="p-2 text-gray-400 hover:text-white hover:bg-gray-800 rounded-lg disabled:opacity-30 disabled:cursor-not-allowed transition-colors" title="Next Step">
                <ChevronRight className="w-5 h-5" />
            </button>
            <div className="w-px h-6 bg-gray-700 mx-1"></div>
            <button onClick={handleReset} className="p-2 text-gray-400 hover:text-red-400 hover:bg-gray-800 rounded-lg transition-colors" title="Reset">
        ...
        <div className="w-2/5 bg-black/95 p-4 font-mono text-xs overflow-y-auto border-r border-gray-800 flex flex-col">
          ...
          <div className="pt-3 border-t border-gray-800 text-gray-500 flex items-center gap-2 justify-between">
             <div className="flex items-center gap-2 overflow-hidden">
                <span className="bg-gray-800 text-gray-300 px-2 py-0.5 rounded text-[10px] shrink-0">Step {Math.min(currentStep + 1, totalSteps)}/{totalSteps}</span>
        ...
        <div className="w-3/5 bg-slate-900 relative p-6 overflow-y-auto flex flex-col items-center">
              ...
              <div className="flex flex-col gap-3 mt-6">
                <div className="flex items-start justify-between">
                    <div className="w-full">
                      <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="font-mono text-lg font-bold text-white tracking-tighter break-all leading-none">{state.mintAccount?.address}</span>
                      </div>
                      <div className="text-[10px] text-gray-400 font-mono"><p>Decimals: {state.mintAccount?.decimals}</p></div>
                    </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="bg-slate-900/50 rounded p-2 border border-slate-700/50 flex flex-col justify-center">
                      <span className="text-[10px] text-gray-500 mb-1 flex items-center gap-1"><ShieldCheck className="w-3 h-3" /> Mint Auth</span>
                      <span className={`text-xs font-mono font-bold break-all leading-tight ${state.localWallet.address === state.mintAccount?.authority ? 'text-blue-300' : 'text-gray-400'}`}>{state.mintAccount?.authority}</span>
                  </div>
                  <div className="bg-slate-900/50 rounded p-2 border border-slate-700/50 flex flex-col justify-center">
                      <span className="text-[10px] text-gray-500 mb-1 flex items-center gap-1"><Snowflake className="w-3 h-3" /> Freeze Auth</span>
                      <span className={`text-xs font-mono font-bold break-all leading-tight ${state.localWallet.address === state.mintAccount?.freezeAuthority ? 'text-blue-300' : 'text-gray-400'}`}>{state.mintAccount?.freezeAuthority}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="flex w-full max-w-4xl justify-between items-start gap-4">
            <div className="flex-1 flex flex-col items-center gap-4 group">
               <div className="bg-slate-800 border border-blue-500/30 rounded-xl p-4 w-full relative z-10 transition-all shadow-lg">
                  ...
                  <div className="flex flex-col mb-2">
                    ...
                    <span className="font-mono text-xs text-gray-400 bg-blue-900/20 px-1.5 py-1 rounded break-all leading-tight">{state.localWallet.address}</span>
                  </div>
               ...
               {state.localATA && <div className="h-6 w-0.5 bg-blue-500/50 relative"><div className="absolute top-1/2 left-2 text-[9px] text-blue-400 whitespace-nowrap -translate-y-1/2">Controls</div></div>}
               <div className={`w-full transition-all duration-500 ${state.localATA ? 'opacity-100 scale-100' : 'opacity-0 scale-95 h-0'}`}>
                 <div className="bg-slate-800/50 border border-green-500/30 border-dashed rounded-xl p-3 w-full relative">
                    <div className="absolute -top-2.5 left-1/2 -translate-x-1/2 bg-slate-900 border border-gray-700 px-2 py-0.5 rounded text-[9px] text-gray-400 uppercase">My Token Account (ATA)</div>
                    <div className="space-y-3">
                        ...
                        <div className="text-[10px] space-y-2 font-mono border-t border-gray-700/50 pt-2">
                            <div className="flex flex-col gap-0.5"><span className="text-gray-500 text-[9px]">ATA Address:</span><span className="text-gray-400 text-[10px] break-all leading-tight">{state.localATA?.address}</span></div>
                            <div className="flex justify-between"><span className="text-gray-500 flex items-center gap-1"><Anchor className="w-3 h-3"/> Rent:</span><span className="text-yellow-500/80">~0.002 SOL (Locked)</span></div>
                            <div className="flex flex-col bg-blue-900/20 p-1.5 rounded border border-blue-500/20 gap-1"><span className="text-blue-400 font-bold flex items-center gap-1 text-[9px]"><Lock className="w-3 h-3" /> Owner (Authority):</span><span className="text-blue-300 font-bold text-[10px] break-all leading-tight">{state.localATA?.owner}</span></div>
                        </div>
                    </div>
                 </div>
               </div>
            ...
            <div className="flex flex-col items-center justify-center pt-24 w-12 shrink-0">
               {state.friendATA?.balance > 0 && <div className="flex flex-col items-center text-green-500 animate-pulse"><div className="text-[9px] mb-1 font-mono">8.8</div><ArrowRight className="w-6 h-6" /></div>}
            </div>
            ...
            <div className="flex-1 flex flex-col items-center gap-4">
               {!state.friendWallet ? (
                   <div className="h-40 w-full border-2 border-dashed border-gray-800 rounded-xl flex flex-col items-center justify-center gap-3 text-gray-600 transition-all opacity-50">
                        <UserPlus className="w-8 h-8" /><span className="text-xs text-center px-4">Waiting for account creation...<br/>(account1.json)</span>
                   </div>
               ) : (
                   <div className="animate-[scaleIn_0.3s_ease-out] w-full flex flex-col items-center gap-4">
                       <div className="bg-slate-800 border border-purple-500/30 rounded-xl p-4 w-full relative z-10 shadow-lg">
                          ...
                          <div className="flex flex-col mb-2">
                            <div className="flex items-center gap-2 mb-1">
                              <Wallet className="w-5 h-5 text-purple-400 shrink-0" /><span className="font-bold text-sm">Friend</span>
                            </div>
                            <span className="font-mono text-xs text-gray-500 bg-purple-900/30 px-1.5 py-1 rounded break-all leading-tight">{state.friendWallet.address}</span>
                          </div>
                       ...
                       {state.friendATA && <div className="h-6 w-0.5 bg-purple-500/50 relative"><div className="absolute top-1/2 right-2 text-[9px] text-purple-400 whitespace-nowrap -translate-y-1/2">Controls</div></div>}
                       <div className={`w-full transition-all duration-500 ${state.friendATA ? 'opacity-100 scale-100' : 'opacity-0 scale-95 h-0'}`}>
                         <div className="bg-slate-800/50 border border-purple-500/30 border-dashed rounded-xl p-3 w-full relative">
                            <div className="absolute -top-2.5 left-1/2 -translate-x-1/2 bg-slate-900 border border-gray-700 px-2 py-0.5 rounded text-[9px] text-gray-400 uppercase">Friend's ATA</div>
                             <div className="space-y-3">
                                <div className="flex justify-between items-end">
                                ...
                                <div className="text-[10px] space-y-2 font-mono border-t border-gray-700/50 pt-2">
                                    <div className="flex flex-col gap-0.5"><span className="text-gray-500 text-[9px]">ATA Address:</span><span className="text-gray-400 text-[10px] break-all leading-tight">{state.friendATA?.address}</span></div>
                                    <div className="flex justify-between"><span className="text-gray-500 flex items-center gap-1"><Anchor className="w-3 h-3"/> Rent:</span><span className="text-yellow-500/80">~0.002 SOL (Locked)</span></div>
                                    <div className="flex flex-col bg-purple-900/20 p-1.5 rounded border border-purple-500/20 gap-1"><span className="text-purple-400 font-bold flex items-center gap-1 text-[9px]"><Lock className="w-3 h-3" /> Owner (Authority):</span><span className="text-purple-300 font-bold text-[10px] break-all leading-tight">{state.friendATA?.owner}</span></div>
                                </div>
                            </div>
                         </div>
                       </div>
          ...
          <div className={`mt-10 w-full max-w-2xl transition-all duration-700 ${state.mintAccount2022 ? 'opacity-100' : 'opacity-0'}`}>
            <div className="border-t border-gray-700 pt-6">
                <div className="flex items-center justify-center gap-2 mb-4"><span className="text-xs font-bold text-gray-500 uppercase">Latest Activity: Token-2022 Extension</span><ArrowDown className="w-3 h-3 text-gray-600" /></div>
                <div className="flex justify-center">
                    <div className="bg-gradient-to-br from-slate-800 to-slate-900 border border-blue-400/30 rounded-xl p-4 w-96 shadow-lg hover:border-blue-400/60 transition-colors relative">
                        <div className="absolute top-2 right-2"><span className="text-[9px] bg-blue-900/30 text-blue-400 px-1.5 py-0.5 rounded border border-blue-700/30 flex items-center gap-1 font-mono"><Cpu className="w-3 h-3" /> Owner: Token-2022</span></div>
                        <div className="flex items-center justify-between mb-3 mt-4">
                            ...
                            <div className="flex gap-1">{state.mintAccount2022?.metadata && <span className="text-[10px] bg-green-900/50 text-green-300 px-2 py-0.5 rounded border border-green-800 flex items-center gap-1"><FileText className="w-3 h-3" /> Metadata</span>}</div>
                        </div>
                        <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                                <div className="text-sm font-bold text-white">{state.mintAccount2022?.metadata?.name || 'Unknown Token'}</div>
                                <div className="text-xs font-bold text-blue-400">{state.mintAccount2022?.metadata?.symbol || '...'}</div>
                                <div className="text-sm text-gray-500 font-mono mt-1 break-all leading-tight">{state.mintAccount2022?.address}</div>
