...
import { Terminal, Wallet, ArrowRight, Database, Coins, HardDrive, Share2, Key, ShieldCheck, ArrowDown, Search, FileText, Lock, UserPlus, Snowflake, Anchor, Cpu, Play, Pause, ChevronLeft, ChevronRight, RotateCcw } from 'lucide-react';
...
const SolanaSimulator = () => {
  ...
  // 使用 CLI 日志中的真实完整地址
  const ADDRESSES = {
    LOCAL_WALLET: '9Uyhkxv2pdS8Zhuxw58pwb5bPJPSHWPvFoL5vuuB6KsJ', // 推断为 Payer
    FRIEND_WALLET: '3MpT32ioVedkZCdPrJKTNGbN2o87BWcgViJ8LRhfGWgn', // 日志 line 4959
    MINT_WOW: 'wowRPZaLK9sCjrqxnbndHg26Kryt8uhNGZNyuoA1yMB',    // 日志 line 4895
    MINT_2022: '6JeeRuSAN3vk7CzUbDEQhbZ5BG57oxMsVhs2CF8G46YE',   // 日志 line 5008
    LOCAL_ATA: 'Ej5qHqT5z9f8a7b6c5d4e3f2g1h0i9j8k7l6m5n4o3p',     // 模拟完整地址
    FRIEND_ATA: '9Au2xY3z5w4v3u2t1s0r9q8p7o6n5m4l3k2j1i0h9g8'      // 模拟完整地址
  };

  const INITIAL_STATE = {
    // 1. 本地钱包 (System Account)
    localWallet: { 
      address: ADDRESSES.LOCAL_WALLET, 
      sol: 0,
      label: 'Local Wallet (Payer)'
    },
    // 2. 朋友钱包 - 初始为 null，等待创建
    friendWallet: null, 

    // 3. 铸币厂账户 (Mint Account) - 初始不存在
    mintAccount: null, 
    // 4. 关联代币账户 (ATAs) - 初始不存在
    localATA: null,
    friendATA: null,
  ...
  const steps = useMemo(() => [
    // --- 阶段一：环境准备 ---
    {
      ...
      desc: '确认当前 CLI 使用的钱包地址 (Line 4882)',
      action: (s) => s, 
    ...
    },
    {
      ...
      desc: '查看当前网络配置 (Devnet/Localhost) (Line 4882)',
      action: (s) => s,
    ...
    // --- 阶段二：生成靓号 & 创建标准 Token ---
    {
      ...
      desc: '生成 "wow" 开头的靓号密钥文件 (Line 4893)',
      action: (s) => s,
    ...
    },
    {
      ...
      desc: '创建 Mint Account (同时设置 Mint 和 Freeze 权限)',
      action: (s) => ({ 
        ...s, 
        ...
        mintAccount: {
          ...
          supply: 0,
          decimals: 9, 
          authority: s.localWallet.address,       // 铸币权限
          freezeAuthority: s.localWallet.address, // 冻结权限 (默认设为创建者)
          type: 'spl'
        }
      }),
    ...
    },
    {
      ...
      desc: '铸造 10 个代币给当前账户 (自动创建 ATA) (Line 4914)',
      action: (s) => ({
        ...s,
    ...
    // --- 阶段三：转账与账户交互 ---
    {
      ...
      desc: '转账并代付租金。注意：0.002 SOL 进入了 ATA 账户而非朋友钱包。',
      action: (s) => ({
        ...s,
        localWallet: { ...s.localWallet, sol: 0.994 }, // 支付对方 ATA 租金 (~0.002) + Gas
    ...
    },
    {
      ...
      desc: '验证接收方余额 (Line 4977)',
      action: (s) => s,
      output: '8.8'
    },

    // --- 阶段四：Token-2022 与 元数据 ---
    {
      id: 11,
      command: 'spl-token create-token --enable-metadata --decimals 0 --program-2022',
      desc: '创建 Token-2022 标准代币 (支持元数据) (Line 5007)',
      action: (s) => ({
        ...s,
        ...
        mintAccount2022: {
          ...
          freezeAuthority: s.localWallet.address, // Token-2022 也有 Freeze Auth
          type: '2022',
          metadata: null
        }
      }),
    ...
    },
    {
      ...
      desc: '初始化链上元数据 (Line 5008)',
      action: (s) => ({
        ...s,
        mintAccount2022: { ...s.mintAccount2022, metadata: { name: 'US-DOllar', symbol: 'USD', uri: 'https://...' } }
      }),
    ...
    },
    {
      ...
      desc: '查看代币展示信息 (CLI 渲染元数据) (Line 5023)',
      action: (s) => s,
      output: 'SPL Token 2022\nName: US-DOllar\nSymbol: USD\nURI: https://raw.githubusercontent.com/...\nDecimals: 0'
    }
  ], []);

  // 核心功能：计算直到 stepIndex 的所有状态
  // 这允许我们随意跳转到任意一步，并保证状态是正确的
  ...
  const handlePrev = () => {
    if (currentStep > 0) {
        jumpToStep(currentStep - 1);
        setIsPlaying(false); // 手动干预时暂停自动播放
    }
  };
  ...
  useEffect(() => {
    let timeout;
    if (isPlaying && !isTyping && currentStep < steps.length - 1) {
        // 打字结束，等待一段时间后自动下一步
        timeout = setTimeout(() => {
            handleNext();
        }, 1500); // 1.5秒阅读时间
    } else if (isPlaying && !isTyping && currentStep === steps.length - 1) {
        setIsPlaying(false); // 结束
    }
    return () => clearTimeout(timeout);
  }, [isPlaying, isTyping, currentStep, steps.length]);
      ...
      <div className="h-14 bg-gray-900 border-b border-gray-800 flex items-center px-4 justify-between shrink-0">
        ...
        <div className="flex items-center gap-2">
            ...
            <button 
                ...
                title={isPlaying ? "暂停 Pause" : "自动播放 Auto Play"}
            >
                {isPlaying ? <Pause className="w-4 h-4 fill-current" /> : <Play className="w-4 h-4 fill-current" />}
                <span className="hidden sm:inline">{isPlaying ? '暂停' : '自动播放'}</span>
            </button>
        ...
        <div className="w-2/5 bg-black/95 p-4 font-mono text-xs overflow-y-auto border-r border-gray-800 flex flex-col">
          ...
          <div className="pt-3 border-t border-gray-800 text-gray-500 flex items-center gap-2 justify-between">
             <div className="flex items-center gap-2 overflow-hidden">
                <span className="bg-gray-800 text-gray-300 px-2 py-0.5 rounded text-[10px] shrink-0">Step {currentStep + 1}/{steps.length}</span>
                <span className="truncate" title={steps[currentStep].desc}>{steps[currentStep].desc}</span>
             </div>
        ...
        <div className="w-3/5 bg-slate-900 relative p-6 overflow-y-auto flex flex-col items-center">
          ...
          <div className={`transition-all duration-700 w-full max-w-2xl mb-8 flex justify-center ${state.mintAccount ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-10'}`}>
            <div className="bg-slate-800 border-2 border-yellow-500/50 rounded-xl p-4 w-96 relative shadow-[0_0_30px_rgba(234,179,8,0.1)]">
              <div className="absolute -top-3 left-4 bg-yellow-600 text-black px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider">
                SPL Mint Account (wow...)
              </div>
              ...
              <div className="flex flex-col gap-3 mt-2">
                <div className="flex items-start justify-between">
                    <div className="w-full">
                      <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="font-mono text-sm font-bold text-white tracking-tighter break-all leading-none">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div className="bg-slate-900/50 rounded p-2 border border-slate-700/50 flex flex-col justify-center">
                      <span className="text-[9px] text-gray-500 mb-1 flex items-center gap-1">
                        <ShieldCheck className="w-3 h-3" /> Mint Auth
                      </span>
                      <span className={`text-[8px] font-mono font-bold break-all leading-tight ${state.localWallet.address === state.mintAccount?.authority ? 'text-blue-300' : 'text-gray-400'}`}>
                  ...
                  <div className="bg-slate-900/50 rounded p-2 border border-slate-700/50 flex flex-col justify-center">
                      ...
                      <span className={`text-[8px] font-mono font-bold break-all leading-tight ${state.localWallet.address === state.mintAccount?.freezeAuthority ? 'text-blue-300' : 'text-gray-400'}`}>
            ...
            <div className="flex-1 flex flex-col items-center gap-4 group">
               ...
               <div className="bg-slate-800 border border-blue-500/30 rounded-xl p-4 w-full relative z-10 transition-all shadow-lg">
                  ...
                  <div className="flex flex-col mb-2">
                    ...
                    <span className="font-mono text-[9px] text-gray-400 bg-blue-900/20 px-1.5 py-1 rounded break-all leading-tight">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="text-[10px] space-y-2 font-mono border-t border-gray-700/50 pt-2">
                            <div className="flex flex-col gap-0.5">
                                ...
                                <span className="text-gray-400 text-[8px] break-all leading-tight">{state.localATA?.address}</span>
                            </div>
                            ...
                            <div className="flex flex-col bg-blue-900/20 p-1.5 rounded border border-blue-500/20 gap-1">
                                ...
                                <span className="text-blue-300 font-bold text-[8px] break-all leading-tight">{state.localATA?.owner}</span>
                            </div>
                        </div>
                    </div>
                 </div>
                   ...
                   <div className="animate-[scaleIn_0.3s_ease-out] w-full flex flex-col items-center gap-4">
                       <div className="bg-slate-800 border border-purple-500/30 rounded-xl p-4 w-full relative z-10 shadow-lg">
                          ...
                          <div className="flex flex-col mb-2">
                            ...
                            <span className="font-mono text-[9px] text-gray-500 bg-purple-900/30 px-1.5 py-1 rounded break-all leading-tight">
                            ...
                            {/* ATA Detail Grid */}
                             <div className="space-y-3">
                                ...
                                <div className="text-[10px] space-y-2 font-mono border-t border-gray-700/50 pt-2">
                                    <div className="flex flex-col gap-0.5">
                                        <span className="text-gray-500 text-[9px]">ATA Address:</span>
                                        <span className="text-gray-400 text-[8px] break-all leading-tight">{state.friendATA?.address}</span>
                                    </div>
                                    ...
                                    <div className="flex flex-col bg-purple-900/20 p-1.5 rounded border border-purple-500/20 gap-1">
                                        ...
                                        <span className="text-purple-300 font-bold text-[8px] break-all leading-tight">{state.friendATA?.owner}</span>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="bg-gradient-to-br from-slate-800 to-slate-900 border border-blue-400/30 rounded-xl p-4 w-96 shadow-lg hover:border-blue-400/60 transition-colors relative">
                        ...
                        <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                                ...
                                <div className="text-[10px] text-gray-500 font-mono mt-1 break-all leading-tight">
                                ...
                                <div className="mt-2 bg-slate-950/50 p-1.5 rounded flex flex-col justify-between border border-gray-700/50 gap-1">
                                     ...
                                     <span className="text-[8px] text-blue-300 font-mono break-all leading-tight">{state.localWallet.address}</span>
                                </div>
                            </div>
                        </div>
                    </div>
